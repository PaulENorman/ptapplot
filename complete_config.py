...
def complete_json(json_path, sort_dir='x'):
    ...
    if isinstance(taps_input, str):
        if taps_input.endswith('.csv') and os.path.exists(os.path.join(base_dir, taps_input)):
    ...
    for i in range(len(df)):
        p1, p2 = (df.iloc[i], df.iloc[i+1]) if i == 0 else \
                 (df.iloc[i-1], df.iloc[i]) if i == len(df)-1 else \
                 (df.iloc[i-1], df.iloc[i+1])
        dx, dy = p2['x']-p1['x'], p2['y']-p1['y']
        nx, ny = dy, -dx
        norm = np.sqrt(nx**2 + ny**2)
        normals.append([float(nx/norm), float(ny/norm), 0.0] if norm > 0 else [0.0, 1.0, 0.0])

    reserved = {'number', 'x', 'y', 'z', 'normals'}
    cp_cols = [c for c in df.columns if c.lower() not in reserved]

    config["taps"] = {
        "number": df["number"].astype(int).tolist() if "number" in df else list(range(1, len(df)+1)),
    ...
    defaults = {"cp_scale": 100.0, "y_range": [0.0, 1.2], "num_ticks": 4,
                "series_colors": ["red", "blue", "green", "purple"], "series_names": cp_cols or ["Series 1"]}
    for k, v in defaults.items():
        config[k] = config.get(k, v)

    if "points_source" in config: del config["points_source"]
...
if __name__ == "__main__":
    import sys
    complete_json(sys.argv[1] if len(sys.argv) > 1 else "demo_data/drivAer/drivAer_top.json")
